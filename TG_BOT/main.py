...
@bot.message_handler(func=lambda message: message.text == "Все заметки")
def note_history(message):
    chat_id = message.chat.id
    ...
    if chat_id in user_data and 'selected_car' in user_data[chat_id]:
        ...
        if car:
            ...
            # Делаем запрос на сервер для получения истории сервиса
            try:
                response = requests.get(f"{config.API_URL_NOTES}?car_id={car_id}")
                if response.status_code == 200:
                    note = response.json()
                    if note:
                        history_message = "История заметок:\n\n"
                        for notes in note:
                            try:
                                # Попробуем преобразовать дату
                                date_object = datetime.strptime(notes['date'], '%Y-%m-%dT%H:%M:%S.%fZ')
                                formatted_date = date_object.strftime('%Y-%m-%d')
                            except (KeyError, ValueError):
                                # Если что-то пошло не так, выводим сообщение о неизвестной дате
                                formatted_date = "Неизвестная дата"
                            ...
                            history_message += (
                                f"Дата: {formatted_date}\n"  
                                f"Заметка: {notes.get('spare_part', 'Не указаны')}\n"
                                f"Фото: {notes.get('images', 'Не загружено') if notes.get('images') else 'Не загружено'}\n\n"
                            )
                        bot.send_message(chat_id, history_message)
                    else:
                        bot.send_message(chat_id, "История заметко отсутствует.")
                else:
                    bot.send_message(chat_id, "Ошибка при получении истории заметок.")
            except Exception as e:
                bot.send_message(chat_id, f"Ошибка: {str(e)}")
        else:
            bot.send_message(chat_id, "Ошибка: выбранный автомобиль не найден.")
